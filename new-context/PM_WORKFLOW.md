...
- **Responsibilities:**
    ...
    - Creates "Sprint Specs" in `new-context/active_sprint.md` for execution.
    - Reviews completed work against requirements.
    - Manages the `new-context/Channel.md` history.

### 🤖 Claude Code (Lead Developer)
- **Primary Goal:** Efficient and correct code execution.
- **Responsibilities:**
    - Reads `new-context/active_sprint.md` to understand the current task.
    - Executes code changes (modifying files, running tests).
    - Updates `new-context/Channel.md` with implementation details and status.
...
2.  Antigravity writes a detailed spec in `new-context/active_sprint.md`.
...
1.  User runs `claude`.
2.  Claude Code reads `new-context/active_sprint.md`.
3.  Claude Code implements the changes.
4.  Claude Code runs verification (tests, linting).
5.  Claude Code appends a "Completion Report" to `new-context/Channel.md`.

### Phase 3: Review (Antigravity)
1.  Antigravity reads `new-context/Channel.md` and verifies the changes.
2.  If satisfied:
    -   Antigravity clears `new-context/active_sprint.md`.
    -   Antigravity updates `TODO.md` (mark done).
3.  If issues found:
    -   Antigravity updates `new-context/active_sprint.md` with feedback/fixes.
    -   Cycle returns to Phase 2.

---

## 3. Communication Channel (`new-context/Channel.md`)
