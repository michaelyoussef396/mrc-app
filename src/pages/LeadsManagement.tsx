...
const LeadsManagement = () => {
  ...
  const loadMoreLeads = async () => {
    // Observe, don't claim, the generation: appending is not superseding. Now
    // that the status filter runs server-side there is no client-side filter
    // left to mask rows that arrive from a filter the user has already left.
    const requestId = loadRequestRef.current;
    setLoadingMore(true);
