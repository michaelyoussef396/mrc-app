...
// Hook: Fetch Recent Activity (new leads + scheduled inspections)
export function useRecentActivity(limit: number = 10) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // 1. Recent new leads (created in last 7 days)
      const weekAgo = new Date();
      weekAgo.setDate(weekAgo.getDate() - 7);

      const { data: newLeads, error: leadsError } = await supabase
        .from('leads')
        .select('id, full_name, lead_source, created_at')
      ...
      for (const lead of newLeads || []) {
        const time = new Date(lead.created_at!).toLocaleTimeString('en-AU', {
          hour: '2-digit',
          minute: '2-digit',
      ...
      // 2. Recently scheduled inspections (bookings created in last 7 days)
      const { data: bookings, error: bookingsError } = await supabase
        .from('calendar_bookings')
        .select('id, title, start_datetime, assigned_to, created_at')
        .gte('created_at', weekAgo.toISOString())
        .order('created_at', { ascending: false })
        .limit(limit);
      ...
      const techIds = [...new Set((bookings || []).map(b => b.assigned_to).filter(Boolean))];
          ...
          if (session) {
            ...
            if (result.success && result.users) {
              result.users.forEach((u: any) => {
                if (techIds.includes(u.id)) {
                  techMap[u.id] = u.full_name || `${u.first_name || ''} ${u.last_name || ''}`.trim() || u.email;
                }
              });
            }
          }
        } catch {
          // Silently fall back to "Technician"
        }
      }

      for (const booking of bookings || []) {
        const techName = booking.assigned_to ? (techMap[booking.assigned_to] || 'Technician') : 'Unassigned';
        const dateStr = new Date(booking.start_datetime).toLocaleDateString('en-AU', {
          day: 'numeric',
          month: 'short',
          timeZone: 'Australia/Melbourne',
        });
        const timeStr = new Date(booking.start_datetime).toLocaleTimeString('en-AU', {
          hour: '2-digit',
          minute: '2-digit',
          timeZone: 'Australia/Melbourne',
        });
