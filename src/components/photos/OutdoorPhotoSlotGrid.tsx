...
import { uploadInspectionPhoto, unplaceOutdoorPhoto, loadUnplacedPhotos } from '@/lib/utils/photoUpload'
...
const OUTDOOR_SLOT_COUNT = 3
const OUTDOOR_SLOT_LABELS = ['Outdoor 1', 'Outdoor 2', 'Outdoor 3']

interface OutdoorPhotoSlotGridProps {
...
export function OutdoorPhotoSlotGrid({
  photos,
  inspectionId,
  onPhotosChanged,
  onPreviewStale,
  loading,
}: OutdoorPhotoSlotGridProps) {
  ...
  async function handlePickExisting(photo: { id: string; signed_url: string; caption: string | null; photo_type: string; area_id: string | null; subfloor_id: string | null }) {
    ...
    try {
      ...
      await recordPhotoHistory({
        ...
        after: { area_id: null, subfloor_id: null, photo_type: 'outdoor' },
      })
  ...
  async function handleConfirmRemove() {
    ...
    setRemoving(true)
    try {
      await unplaceOutdoorPhoto(photo.id)
      setMode({ step: 'idle' })
      toast.success('Photo removed from outdoor')
      await onPhotosChanged()
      onPreviewStale()
    } catch (err) {
  ...
  return (
    <>
      <div ref={gridRef} className="grid grid-cols-3 gap-3">
        {Array.from({ length: OUTDOOR_SLOT_COUNT }, (_, index) => {
            ...
            return (
              <div key={`slot-${index}`}>
                <button
                  ...
                  aria-label={`${isSelected ? 'Deselect' : 'Select'} ${OUTDOOR_SLOT_LABELS[index]}: ${photo.caption || 'untitled'}`}
                  aria-pressed={isSelected}
                >
                  <img
                    src={photo.signed_url}
                    alt={photo.caption || OUTDOOR_SLOT_LABELS[index]}
                ...
                <p className={`mt-1 text-xs line-clamp-1 ${isDimmed ? 'text-gray-300' : 'text-gray-500'}`}>
                  {photo.caption || OUTDOOR_SLOT_LABELS[index]}
                </p>
                ...
                {isSelected && (
                  <div className="flex items-center justify-center mt-2">
                    <button
                      ...
                      aria-label="Remove photo from outdoor"
                    >
          ...
          return (
            <div key={`slot-${index}`} className={`${isDimmed ? 'opacity-30' : ''}`}>
              <div className="aspect-square rounded-lg border-2 border-dashed border-gray-300 flex flex-col items-center justify-center gap-2 bg-gray-50">
                <span className="text-xs font-medium text-gray-400">{OUTDOOR_SLOT_LABELS[index]}</span>
                {uploading ? (
                  <Loader2 className="h-5 w-5 animate-spin text-gray-400" />
                ) : (
                  <div className="flex gap-2">
                    <button
                      ...
                      title={`Upload ${OUTDOOR_SLOT_LABELS[index]} photo`}
                      aria-label={`Upload photo for ${OUTDOOR_SLOT_LABELS[index]}`}
                    >
                    ...
                    <button
                      ...
                      title={`Pick existing for ${OUTDOOR_SLOT_LABELS[index]}`}
                      aria-label={`Pick existing photo for ${OUTDOOR_SLOT_LABELS[index]}`}
                    >
