...
export function InvoicePaymentCard({ leadId, leadStatus, onRefresh }: Props) {
  const { invoice, isLoading, markPaid, isMarkingPaid, isMarkingSent, isOverdue, daysUntilDue, daysPastDue, refetch, markSent, penaltyTier } =
    ...
    <>
      <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            {penaltyTier && (penaltyTier.warrantyVoid || penaltyTier.warrantySuspended) && (
              <Badge
                className={
                  penaltyTier.warrantyVoid
        ...
        {isSent && <PenaltyLadderWidget invoice={invoice} />}
        ...
        >
          <ExternalLink className="h-4 w-4 mr-1" />Open full invoice editor
        </Button>
