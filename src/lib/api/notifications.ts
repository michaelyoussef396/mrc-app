...
export function buildInvoiceEmailHtml(params: InvoiceEmailParams): string {
  const itemsHtml = params.lineItems.map(item => `
    <tr>
      <td style="padding:8px;border-bottom:1px solid #eee;">${escapeEmailHtml(item.description)}</td>
      <td style="padding:8px;border-bottom:1px solid #eee;text-align:center;">${item.quantity}</td>
      <td style="padding:8px;border-bottom:1px solid #eee;text-align:right;">${formatAUD(item.unit_price)}</td>
      <td style="padding:8px;border-bottom:1px solid #eee;text-align:right;">${formatAUD(item.total)}</td>
    </tr>
  `).join('');

  const discountRow = params.discountAmount > 0
    ? `<tr><td colspan="3" style="padding:8px;text-align:right;">Discount applied</td><td style="padding:8px;text-align:right;color:#c44;">-${formatAUD(params.discountAmount)}</td></tr>`
    : '';

  const notesHtml = params.notes
    ? `<p style="margin-top:16px;"><strong>Notes:</strong> ${escapeEmailHtml(params.notes)}</p>`
    : '';

  return wrapInBrandedTemplate(`
    <h2>Invoice ${escapeEmailHtml(params.invoiceNumber)}</h2>
    <p>Dear ${escapeEmailHtml(params.customerName)},</p>
    <p>Thank you for choosing Mould &amp; Restoration Co. Please find your invoice details below.</p>

    <div class="details-box">
      <table>
        <tr><td>Invoice Number</td><td><strong>${escapeEmailHtml(params.invoiceNumber)}</strong></td></tr>
        <tr><td>Invoice Date</td><td>${escapeEmailHtml(params.invoiceDate)}</td></tr>
        <tr><td>Due Date</td><td><strong>${escapeEmailHtml(params.dueDate)}</strong></td></tr>
        <tr><td>Property</td><td>${escapeEmailHtml(params.propertyAddress)}</td></tr>
      </table>
    </div>
    ...
    <table style="width:100%;border-collapse:collapse;margin-top:12px;">
      ...
      <tfoot>
        <tr><td colspan="3" style="padding:8px;text-align:right;">Subtotal (ex GST)</td><td style="padding:8px;text-align:right;">${formatAUD(params.subtotal)}</td></tr>
        ${discountRow}
        <tr><td colspan="3" style="padding:8px;text-align:right;">GST (10%)</td><td style="padding:8px;text-align:right;">${formatAUD(params.gstAmount)}</td></tr>
        <tr style="background:#f5f5f5;font-weight:bold;">
          <td colspan="3" style="padding:10px;text-align:right;font-size:16px;">Total (inc GST)</td>
          <td style="padding:10px;text-align:right;font-size:16px;">${formatAUD(params.totalAmount)}</td>
        </tr>
      </tfoot>
    </table>

    <h3 style="margin-top:32px;">Payment Details</h3>
    <div class="details-box">
      <table>
        <tr><td>BSB</td><td>[TO BE CONFIRMED]</td></tr>
        <tr><td>Account Number</td><td>[TO BE CONFIRMED]</td></tr>
        <tr><td>Account Name</td><td>Mould &amp; Restoration Co.</td></tr>
        <tr><td>Reference</td><td><strong>${escapeEmailHtml(params.invoiceNumber)}</strong></td></tr>
      </table>
    </div>

    <p><strong>Accepted payment methods:</strong> Cash, Visa, Mastercard, Bank Transfer, Cheque.</p>

    ${notesHtml}

    <p style="margin-top:24px;color:#c44;"><strong>Please note:</strong> Payment is due by ${escapeEmailHtml(params.dueDate)}. Late payments may incur additional charges as per our Terms &amp; Conditions.</p>

    <p>If you have any questions, please contact us on <strong>1800 954 117</strong> or reply to this email.</p>
  `);
}
...
export async function sendInvoiceEmail(params: {
  leadId: string;
  customerEmail: string;
  emailParams: InvoiceEmailParams;
}): Promise<void> {
  const html = buildInvoiceEmailHtml(params.emailParams);
  await sendEmail({
    to: params.customerEmail,
    subject: `Invoice ${params.emailParams.invoiceNumber} — Mould & Restoration Co.`,
    html,
    leadId: params.leadId,
