...
- **Diff limit (the one copy — every other file points here):** **150 SOURCE lines per reviewed unit.** A unit is the range one review reads: a commit or a contiguous run of commits on one branch, declared before the run. Count mechanically from the worktree root, excluding `docs/sessions/`, and split source from test: `git diff --numstat <base>...<head> -- . ':(exclude)docs/sessions/' | awk '$3 ~ /(__tests__\/|\.test\.|\.spec\.)/ {t+=$1+$2; next} {s+=$1+$2} END {print "source=" s+0, "test=" t+0}'`. Source is capped at 150; tests are reported and unlimited. Above the cap: split the unit, or get a waiver from Michael BEFORE the run. Two units reviewed separately may land in one PR; that is the ordinary path, not a waiver. Every waiver line written anywhere (session log, ledger) carries a UTC timestamp and the SHA it precedes. History: the 2026-09-09 NO RAISE ruling on 365 total lines stands as recorded; the per-source rule applies from 2026-09-15.
- **A review cannot be path-scoped. The cap is managed by splitting commits, never by narrowing what Codex reads.** Verified in the plugin source 2026-09-09 (`openai-codex/codex/1.0.6`): `scripts/codex-companion.mjs:714` accepts only `base`, `scope`, `model` and `cwd`, and `scripts/lib/git.mjs:279` runs `git diff --binary --no-ext-diff --submodule=diff <commitRange>` with **no pathspec** and no env override — on the inline path, which the companion takes only when the diff touches at most 2 files and is under 256 KiB (`git.mjs:8`, `:332`); for 3+ files it sends only the commit log, diff stat and changed-files list (equally unscoped, `:266`) and Codex fetches the diff itself (self-collect). Either way nothing narrows what is reviewed. There is no flag that excludes `__tests__` or reviews source only, and any extra token you pass becomes focus text instead — the same mechanism as the `--help` trap in `docs/CODEX_WORKFLOW.md` §3. Do not go looking for one. Splitting a unit for *review* is not the same as merging it in pieces.
- **Companion input mode is invisible in its output.** The plugin's scripts are vendor-owned and overwritten on upgrade, so the companion is not edited to print it. Instead the lane procedure prints the changed-file count before the run; 3 or more files means self-collect.
- **No customer PII** in anything sent to Codex. Check the diff first.
- **Focus text for the Codex review script is ALWAYS single-quoted, and the focus string is printed back before any finding is read.** The companion script takes everything after `--` as focus text, but the Bash tool runs the line through zsh first, so an unquoted `;` inside the focus is a command separator: only the clause before the first `;` reaches Codex, and every clause after it runs as a shell command. **The review still returns a `Target:` line and a verdict**, so a review that examined a fraction of what was asked is indistinguishable from a complete one — the only visible trace is a non-zero exit and `command not found:` lines mixed into the output, which are easy to read as harmless noise. Observed live 2026-09-08 on `fix/quote-equipment-days-pdf`: a four-clause focus was cut to one, three clauses executed as commands, `exit 127`, and round 1 returned a clean approve anyway; a redirect written after the focus also attached itself to a stray fragment instead of the node command, so the stderr capture holding the thread id was lost. Print the focus string verbatim alongside the `Target:` line and the reviewable line count, before reading any finding — the same discipline, for the same reason: all three are cheap to check and all three fail silently.
...
- Migration policy lives in the headers of the `.sql` files. Read them before commenting on migration ordering. Do not advise on ordering from general knowledge. Never bulk-UPDATE `public.inspections` in a migration or script: the BEFORE UPDATE trigger stamps `updated_at` on every touched row, which hides every stalled inspection from the dashboard tile for 24 hours.
...
- **The reviewable-diff limit.** The threshold, the unit, the pathspec and the exact command are under Non-negotiable rules above (Diff limit), the one copy; `docs/CODEX_WORKFLOW.md` §4 and `CLAUDE.md` step 2 point there. They are deliberately not repeated here, so there is no second copy to drift. Over the limit, split the unit. Never subtract from the count to get under it — the byte-identical-restore waiver is Michael's to apply, not an agent's.
