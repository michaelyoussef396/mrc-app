...
- **A review cannot be path-scoped. The cap is managed by splitting commits, never by narrowing what Codex reads.** Verified in the plugin source 2026-09-09 (`openai-codex/codex/1.0.6`): `scripts/codex-companion.mjs:714` accepts only `base`, `scope`, `model` and `cwd`, and `scripts/lib/git.mjs:279` runs `git diff --binary --no-ext-diff --submodule=diff <commitRange>` with **no pathspec** and no env override. There is no flag that excludes `__tests__` or reviews source only, and any extra token you pass becomes focus text instead — the same mechanism as the `--help` trap in `docs/CODEX_WORKFLOW.md` §3. Do not go looking for one. **Report the measured count split into source and test lines** when you report it: a total inflated by test scaffolding is a different decision from one inflated by production logic, and the split is what makes that visible. Splitting a unit for *review* is not the same as merging it in pieces — two commits reviewed separately can still land in one PR, and that is the way out of a cap, not a waiver.
