...
| 1 | N | Node 24 pin, deadline 1 Oct | `~/mrc-node22` | `chore/node-24` | `package.json` engines, `.nvmrc`, the Node version lines in `docs/DEPLOYMENT.md` and `docs/DEVELOPER-GUIDE.md` | everything else; the Vercel project setting is Michael's | everything |
...
|---|---|---|---|---|---|
| N | Node 24 pin — scoped as "Node 22", shipped as 24. Vercel disables Node 20 on 1 Oct for Builds and Functions: **new deployments error, already-deployed functions keep serving**, so it is a deploy-blocker, not an outage. What selects the version is `engines.node` in `package.json` and the Vercel dashboard setting, and **`engines.node` overrides the dashboard**. **`.nvmrc` is inert on Vercel** — Vercel has never read it; it steers local `nvm use` only and must not be described as the fix. Pinned `24.x` rather than `22.x`: 24 is Vercel's default and the only Active LTS (EOL Apr 2028 vs Apr 2027 for 22), zero of 1147 lockfile entries exclude it against four with floors inside 22.x (`@sparticuz/chromium` needs >=22.17.0), and `e2a6b7a` records the two Chromium PDF functions already running on `nodejs24.x` — so 22 would have been a downgrade. The Vercel project setting is Michael's. | [ ] | `chore/node-24` / N | 2026-09-05 local only: `npm ci` + `npm run build` + `npx tsc -p tsconfig.app.json --noEmit` all green on Node v24.20.0 / npm 11.19.0, 99→99 error lines byte-identical, zero EBADENGINE. **Not preview-verified** — still needs the dashboard value, a green deploy and one PDF render. Local green cannot prove the api/ functions survive the bump; see T13. | n/a |
