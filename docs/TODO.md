...
- **PDF-CL18 — `20260826120001`'s STATUS header claims a `CREATE POLICY` path on `storage.objects` that the `postgres` role cannot actually use.** Its header (`:4-7`) records three `CREATE POLICY` statements applied to PROD "via `npx supabase db query --linked -f`", i.e. as `postgres`. Measured live on **both** projects 2026-08-28: `storage.objects` is owned by `supabase_storage_admin`, and `pg_has_role('postgres', relowner, 'USAGE')` returns **false** — so `postgres` cannot do policy DDL on that table and such a statement raises 42501. (`postgres` does have `rolbypassrls = true`, but that exempts it from RLS on DML, not from the ownership requirement for DDL.) The three `lead_note_attachments_object_*` policies **do** exist live, confirmed in a full `pg_policies` capture, so they were created somehow — most likely through Dashboard → Storage → Policies, with the header recording the intent rather than the mechanism. **Why it matters:** that header is the reason `20260827210000_pdf_bucket_write_policies_lock_down.sql` was initially written treating 42501 as unlikely; it had to be corrected to make the Dashboard the primary apply path. A migration header that misdescribes how it was applied will mislead the next storage change the same way. Action: establish how those three policies actually landed and correct the header. Low effort, docs-accuracy only — **not a blocker**.
