...
| `generate-inspection-summary` | `ai_summary_versions` INSERT (new version row per generation) + `ai_summary_versions` UPDATE (supersession of previous active version) | `src/pages/InspectionAIReview.tsx` and `src/pages/TechnicianInspectionForm.tsx` via `invokeEdgeFunction()` | **JWT-bound client `supabaseAudited`** built from forwarded `Authorization` header — `audit_log_trigger` captures `auth.uid()` for both INSERT and UPDATE. EF also accepts an explicit `userId` in payload (Phase 2 propagation pattern) and writes it to `ai_summary_versions.generated_by`. Race safety on `UNIQUE(inspection_id, version_number)` via retry loop, not RPC. **Updated 2026-05-02 — Phase 3 Stage 3.2 promoted this EF from "no audited write" to a full audited writer.** |
