...
- **T20:** the four `VITE_` Production env vars and the bundle Supabase ref were not verified around the PR #157 promote.
- **`~/mrc-app-1` still holds the abandoned `87096a7` on `chore/workflow-scaffolding`** — the first attempt, same content, superseded by the commit on `main`. It is unpushed and must stay that way: pushing that branch would open a second PR duplicating what is already on `main`. Drop it whenever convenient; nothing depends on it.
...
- Updated: 2026-09-08 10:17 AEST · Tool: CC
- Branch: chore/workflow-scaffolding @ 87096a7 docs: file six untracked team requests, correct the production gap
- Unpushed commits:
  - `87096a7 docs: file six untracked team requests, correct the production gap`
...
- Last step-log line: 10:15 · CC · CC · SELF-INFLICTED, caught and fixed in the same turn: the first T21 edit put three unescaped `|` inside a table cell (quoted grep patterns), splitting the row into 9 columns. Escaped as `\|` and re-scanned every table row in the file; the only remaining non-6-column rows are the pre-existing session-plan tables and P0-9, none of them touched here · docs/TODO.md · fixed, verified by full-file column scan
- Codex threads: none
