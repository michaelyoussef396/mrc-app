...
Original implementation is committed as `227cd2e`, and its retry/test scope is accepted. **Do not deploy that failure policy as resolved:** the answer above recommends P0-A-style degraded success for the legacy EF. Michael asked for this answer before implementation; no behavior change has been made in the follow-up. Keep the API hard-save policy intact. This follow-up edits only the log. PDF-CL17 remains read-only with an explicitly unproven cause; Claude review and Michael-applied deployment remain pending.
