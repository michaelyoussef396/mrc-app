...
# one start and one end marker, in that order, is never edited: nothing is written and one notice
# per session is emitted (a legacy log gets the pair by hand, once). Missing jq/git/sessions dir,
# a read-only log, or a log changed mid-run: exit 0 silently. docs/CODEX_WORKFLOW.md §10.
...
last_step_line() {  # a READ: last "- " line of the step log; a wrong match (a bullet quoted inside a fenced example) is cosmetic
  awk -v h="$STEP_HEADING" '{ sub(/\r$/, "") } $0 ~ "^" h "[[:space:]]*$" { s = 1; next } s && /^## / { exit } s && /^- / { l = substr($0, 3) } END { print l }' "$1"
}
