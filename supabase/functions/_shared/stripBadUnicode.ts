 ...
 * `ai_summary_versions` (persistVersionRow + the structured-output parse step).
 *
 * SYNC: keep identical to src/lib/stripBadUnicode.ts. Two copies exist only because
 * the Vite frontend and the Deno edge function cannot import a single shared module
 * — do not let them drift. No `u` flag: the surrogate handling relies on UTF-16
 * code-unit semantics.
 */
